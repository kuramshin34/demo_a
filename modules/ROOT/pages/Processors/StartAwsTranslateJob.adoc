= StartAwsTranslateJob 2.2.0

Запускает задание AWS Translate. Для отслеживания статуса задания необходимо использовать процессор GetAwsTranslateJobStatus.
...
|`original`
|После успешной обработки оригинальный FlowFile направляется в это отношение.
...
|`awsTaskId`
|Идентификатор задания, который можно использовать для опроса завершения задания в GetAwsTranslateJobStatus
