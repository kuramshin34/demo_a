...
Извлекает документ из DynamoDB на основе хеш- и диапазонного ключа. Ключ может быть строкой или числом. Для любого запроса по получению требуются все основные ключи (хеш или хеш и диапазон в зависимости от ключей таблицы). Поле JSON ('Map') атрибута элемента DynamoDB считывается в содержимое FlowFile.
