= ScriptedRecordSink 2.2.0

Позволяет пользователю предоставить экземпляр Scripted RecordSinkService для передачи записей в целевой объект. Сценарий должен установить переменную 'recordSink' на реализацию RecordSinkService.
