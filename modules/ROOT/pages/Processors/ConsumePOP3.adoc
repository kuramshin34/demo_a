...
Потребляет сообщения из Email Server с использованием протокола POP3. Необработанные байты каждого полученного электронного письма записываются как содержимое FlowFile
...
|`success`
|Все сообщения, успешно полученные с Email сервера и преобразованные в FlowFiles, направляются по этому отношению
