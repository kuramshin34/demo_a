= GetAwsTranslateJobStatus 2.2.0

Получает текущий статус задания AWS Translate.
...
|`throttled`
|Результаты не удалось получить по какой-то причине, но проблема, скорее всего, решится сама собой, например из-за Предоставленного Throughput или сбоя ограничения. Обычно ожидается повторная попытка этого отношения.
...
|`outputLocation`
|S3 путь-стиль выходного расположения результата.
